...
#include <renderer/rhi/vulkanresourcecontext.h>
...
namespace engine
{
    namespace vulkan
    {
        ...
        using engine::resources::CRHIResourceManager;
        ...
        namespace detail
        {
                                  ...
                                  , CRenderGraphMutableResources     const &aRenderGraphResources) -> EEngineStatus
            {
                ...
                auto const addIfNotAddedYet =
                               [](Vector<SRHISubpassDependency> &aDependencies, SRHISubpassDependency const &aToBeInserted) -> void
                                   {
                ...
                for(std::size_t k=0; k<aPassExecutionOrder.size(); ++k)
                {
                    ...
                    SRHISubpassDescription subpassDesc = {};
                    ...
                    for(auto const &index : attachmentResourceIndexList)
                    {
                        ...
                        SRHIAttachmentReference attachmentReference {};
                        attachmentReference.attachment = static_cast<uint32_t>(attachmentIndex);

                        SRHISubpassDependency dependency = {};
                        ...
                        if(0 == (alreadyProcessedAttachmentsFlags & (1u << attachmentIndex)))
                        {
                            ...
                            SRHIAttachmentDescription attachmentDesc = {};
                ...
                std::array<SRHISubpassDependency, 2> finalDependencies = {};
                ...
                {
                    CEngineResult<OptRef_t<RHIRenderPassResourceState_t>> renderPassObject = aResourceManager->useResource<RHIRenderPassResourceState_t>(renderPassDesc.name, renderPassDesc, true);
            ...
            {
                CEngineResult<> const deinitialized = aResourceManager->discardResource<RHIRenderPassResourceState_t>(aRenderPassId, aVulkanEnvironment.get());
            ...
            auto createFrameBuffer(Shared<CVulkanEnvironment>      aVulkanEnvironment
                                   ...
                                   , ResourceId_t           const &aRenderPassId) -> EEngineStatus
            {
                SRHIFrameBufferDescription desc {};
                desc.name                 = aFrameBufferId;
                desc.renderPassResourceId = aRenderPassId;
                {
                    CEngineResult<OptionalRef_t<RHIFrameBufferResourceState_t>> frameBufferObject = aResourceManager->useResource<RHIFrameBufferResourceState_t>(desc.name, desc, true);
            ...
            {
                CEngineResult<> const deinitialized = aResourceManager->discardResource<RHIFrameBufferResourceState_t>(aFrameBufferId, aVulkanEnvironment.get());
            ...
            {
                SRHIImageDescription desc = {};
                desc.name      = aTexture.readableName;
                desc.imageInfo = static_cast<graphicsapi::STextureInfo>(aTexture.description.dynamicImage);
                // Always set those...
                ...
                {
                    CEngineResult<RHIResourceState_t> textureObject = aResourceManager->useResource<RHIResourceState_t>(desc.name, desc, true);
            ...
            auto destroyTransientTexture(Shared<CVulkanEnvironment>     aVulkanEnvironment
                                         ...
                                         , SRenderGraphImage const     &aTexture) -> EEngineStatus
            {
                CEngineResult<> const deinitialized = aResourceManager->discardResource<RHIResourceState_t>(aTexture.readableName, aVulkanEnvironment.get());
            ...
            {
                {
                    CEngineResult<> const status = aResourceManager->initializeResource<RHIResourceState_t>(aTexture.readableName, aVulkanEnvironment.get());
            ...
            {
                {
                    CEngineResult<> const status = aResourceManager->updateResource<RHIResourceState_t>(aTexture.readableName, aVulkanEnvironment.get());
            ...
            {
                {
                    CEngineResult<> const status = aResourceManager->deinitializeResource<RHIResourceState_t>(aTexture.readableName, aVulkanEnvironment.get());
            ...
            {
                SRHIImageViewDescription desc = { };
                desc.name             = aView.readableName;
                desc.imageFormat      = aView.description.format;
                desc.subjacentImageId = aTextureName;
                desc.arraySlices      = aView.description.arraySliceRange;
                desc.mipMapSlices       = aView.description.mipSliceRange;

                CEngineResult<RHIImageViewResourceState_t> textureViewObject = aResourceManager->useResource<RHIImageViewResourceState_t>(desc.name, desc, true);
            ...
            {
                CEngineResult<> const deinitialized = aResourceManager->discardResource<RHIImageViewResourceState_t>(aTextureView.readableName, aVulkanEnvironment.get());
                                     ...
                                     , SRenderGraphBuffer const    &aBuffer) -> EEngineStatus
            {
                ...
                CEngineResult<RHIBufferResourceState_t> bufferObject = aResourceManager->useResource<RHIBufferResourceState_t>(desc.name, desc, true);
            ...
            auto destroyTransientBuffer(Shared<CVulkanEnvironment>    aVulkanEnvironment
                                      ...
                                      , SRenderGraphBuffer const     &aBuffer) -> EEngineStatus
            {
                CEngineResult<> const deinitialization = aResourceManager->discardResource<RHIBufferResourceState_t>(aBuffer.readableName, aVulkanEnvironment);
            ...
            {
                {
                    auto const &[successCode] = aResourceManager->initializeResource<RHIBufferResourceState_t>(aBuffer.readableName, aVulkanEnvironment);
            ...
            auto updatePersistentBuffer(Shared<CVulkanEnvironment>   aVulkanEnvironment
                                      ...
                                      , SRenderGraphBuffer const    &aBuffer) -> EEngineStatus
            {
                CEngineResult<> const bufferTransfer = aResourceManager->uploadResource<RHIBufferResourceState_t>(aBuffer.readableName, aVulkanEnvironment);
            ...
            {
                {
                    auto const &[successCode] = aResourceManager->deinitializeResource<RHIResourceState_t>(aBuffer.readableName, aVulkanEnvironment);
            ...
            {
                SRHIBufferViewDescription desc = { };
                ...
                auto const &[success, resource] = aResourceManager->useResource<RHIBufferViewResourceState_t>(desc.name, desc);
            ...
            auto destroyBufferView(Shared<CVulkanEnvironment>      aVulkanEnvironment
                                   ...
                                   , SRenderGraphBufferView  const &aView) -> EEngineStatus
            {
                CEngineResult<> const deinitialization = aResourceManager->discardResource<RHIBufferViewResourceState_t>(aView.readableName, aVulkanEnvironment);
                                ...
                                , SRenderGraphMesh        const &aMesh) -> EEngineStatus
            {
                ...
                auto const &[attributeBufferOpSuccessCode] = aResourceManager->initializeResource<RHIBufferResourceState_t>(mesh.rhiCreateDesc.attributeBufferDesc.name, aVulkanEnvironment);
                            ...
                            , SRenderGraphMesh        const &aMesh) -> EEngineStatus
            {
                ...
                auto const &[attributeBufferOpSuccessCode] = aResourceManager->updateResource<RHIBufferResourceState_t>(mesh.rhiCreateDesc.attributeBufferDesc.name, aVulkanEnvironment);
                if(CheckEngineError(attributeBufferOpSuccessCode))
                {
                    return EEngineStatus::InitializationError;
                }
                                ...
                                , SRenderGraphMesh        const &aMesh) -> EEngineStatus
            {
                ...
                auto const &[attributeBufferOpSuccessCode] = aResourceManager->deinitializeResource<RHIBufferResourceState_t>(mesh.rhiCreateDesc.attributeBufferDesc.name, aVulkanEnvironment);
                if(CheckEngineError(attributeBufferOpSuccessCode))
                {
                    return EEngineStatus::InitializationError;
                }
            ...
            {
                auto const result0 = aResourceManager->initializeResource<RHIPipelineResourceState_t>(aMaterial.description.sharedMaterialResourceId, aVulkanEnvironment);
                // auto const result1 = aResourceManager->initializeResource<MaterialResourceState_t>(aMaterial.rhiCreateDesc.materialResourceId, aVulkanEnvironment);

                // for(auto const &buffer : aMaterial.rhiCreateDesc.buffers)
                //     auto const result2 = aResourceManager->initializeResource<RHIBufferResourceState_t>(buffer.bufferResourceId, aVulkanEnvironment);
                // for(auto const &image : aMaterial.rhiCreateDesc.images)
                //     auto const result3 = aResourceManager->initializeResource<RHIResourceState_t>(image.imageId);
                              ...
                              , SRenderGraphMaterial    const &aMaterial) -> EEngineStatus
            {
                ...
                // auto const result1 = aResourceManager->updateResource<MaterialResourceState_t>(aMaterial.rhiCreateDesc.materialResourceId, aVulkanEnvironment);

                // for(auto const &buffer : aMaterial.rhiCreateDesc.buffers)
                //     auto const result2 = aResourceManager->updateResource<RHIBufferResourceState_t>(buffer.bufferResourceId, aVulkanEnvironment);
                // for(auto const &image : aMaterial.rhiCreateDesc.images)
                //     auto const result3 = aResourceManager->updateResource<RHIResourceState_t>(image.imageId);
                                    ...
                                    , SRenderGraphMaterial    const &aMaterial) -> EEngineStatus
            {
                ...
                // auto const result1 = aResourceManager->deinitializeResource<MaterialResourceState_t>(aMaterial.rhiCreateDesc.materialResourceId, aVulkanEnvironment);

                // for(auto const &buffer : aMaterial.rhiCreateDesc.buffers)
                //     auto const result2 = aResourceManager->deinitializeResource<RHIBufferResourceState_t>(buffer.bufferResourceId, aVulkanEnvironment);
                // for(auto const &image : aMaterial.rhiCreateDesc.images)
                //     auto const result3 = aResourceManager->deinitializeResource<RHIResourceState_t>(image.imageId);
            ...
            //
            //<-----------------------------------------------------------------------------
            auto initializePipeline(Shared<CVulkanEnvironment>       aVulkanEnvironment
                                    , Shared<CRHIResourceManager>       aResourceManager
                                    , Shared<asset::CAssetStorage>   aAssetStorage
                                    ...
                                    , uint32_t const                &aSubpassIndex) -> EEngineStatus
            {
                ...
                aResourceManager->initializeResource<RHIPipelineResourceState_t>(aPipeline.pipelineResourceId, aPipeline.);
            ...
            //
            //<-----------------------------------------------------------------------------
            auto deinitializePipeline(Shared<CVulkanEnvironment>      aVulkanEnvironment
                                      , Shared<CRHIResourceManager>      aResourceManager
                                      , Shared<asset::CAssetStorage>  aAssetStorage
