    ...
    namespace mesh
    {
                                                                         ...
                                                                         , SMeshDataFile              const &aDataFile)
        {
            ...
            DataSourceAccessor_t indexDataAccessor = [=]() -> ByteBuffer
            {
                ...
                ByteBuffer bufferView = buffer.createView(aDataFile.indices
                                                                   .offset, aDataFile.indices
                                                                                     .length);
                return buffer;
            };
    ...
    namespace resources
    {
        ...
        public:
            static SMeshDescriptor derive(Shared<CAssetStorage> aAssetStorage, Shared<CMeshInstance> aInstance)
            {
                ...
                SMeshDataFile const &dataFile = aInstance->dataFile();

                auto [derivationSuccessful, vertexBufferDescription, indexBufferDescription] = deriveResourceDescriptions(aAssetStorage, aInstance->name(), aInstance->dataFile());
