...
namespace engine
{
    namespace gfxapi
    {
        ...
        CEngineResult<Shared<T>> const CGFXAPIResourceBackend::getResource(PublicResourceId_t const &aId)
        {
            ...
            if(not contained)
            {
                CLog::Warning(logTag(), "Resource '{}' not found.", aId);
            ...
            if(nullptr == result)
            {
                CLog::Error(logTag(), "Resource '{}' not of expected type.", aId);
                ...
                Shared<IAsyncLoadCallback>  const &aCallback)
        {
            ...
            {
                CLog::Error(logTag(), CString::format("Failed to access future shared state. Error: {}", fe.what()));
                return CEngineResult<>(EEngineStatus::Error);
            }
            catch(std::exception const &e)
            {
                CLog::Error(logTag(), CString::format("Error while trying to request a task result in the resource backend. Error: {}", e.what()));
                return CEngineResult<>(EEngineStatus::Error);
                ...
                SDeferredResourceOperationHandle           &aOutHandle)
        {
            ...
            {
                CLog::Error(logTag(), CString::format("Failed to create build task for resource '{}'", "..."));
                return creationOp;
            }
            ...
            {
                CLog::Error(logTag(), CString::format("Failed to enqueue resource creation task for resource '{}'", "..."));
                return enqueueing;
            }
                ...
                SDeferredResourceOperationHandle              &aOutHandle)
        {
            ...
            {
                CLog::Error(logTag(), CString::format("Failed to create destruction task for resource '{}'", "..."));
