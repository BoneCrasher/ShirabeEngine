       ...
       public_methods:
        ...
        CEngineResult<> CResourceManagerBase::unloadResourceImpl(std::string const &aResourceId)
        {
            ...
            EngineStatusPrintOnError(
                        status.result(),
                        logTag(),
                        CString::format("Failed to unload proxy resource in backend (Id: {}).", aResourceId));
        ...
        CEngineResult<> CResourceManagerBase::destroyResourceImpl(std::string const &aResourceId)
        {
            ...
            {
                CLog::Error(logTag(), CString::format("Error while trying to destroy a resource in the resource manager. Error: {}", e.what()));
                unload = eraseFn();
            }
            catch(...)
            {
