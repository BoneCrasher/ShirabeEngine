    ...
    namespace vulkan
    {
                ...
                ResourceTaskFn_t                     &aOutTask)
        {
            ...
            aOutTask = [=] () -> CEngineResult<SGFXAPIResourceHandleAssignment>
            {
                ...
                if(not buffer)
                {
                    CLog::Error(logTag(), CString::format("Invalid internal data provided for buffer creatopm. Vulkan error: {}", VkResult::VK_ERROR_INVALID_EXTERNAL_HANDLE));
                ...
                ResourceTaskFn_t                        &aOutTask)
        {
            ...
            aOutTask = [=] () -> CEngineResult<SGFXAPIResourceHandleAssignment>
            {
                ...
                {
                    CLog::Error(logTag(), CString::format("Invalid internal data provided for buffer view destruction. Vulkan error: {}", VkResult::VK_ERROR_INVALID_EXTERNAL_HANDLE));
                    return { EEngineStatus::Error };
                }
