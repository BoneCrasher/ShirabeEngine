          ...
          HandleEngineStatusError(status, "Failed to retrieve device capabilities.");
              ...
              = [this, &configuration] () -> void
            {
              ...
              if(!((adapterIndex + outputIndex + modeIndex) >= 0)) {
                ...
                if(outputIndex < 0)
                  HandleEngineStatusError(EEngineStatus::GAPI_NoPrimaryAdapterOutputs, String::format("No valid outputs for adapter @index {} available.", adapterIndex));
                if(modeIndex < 0)
                  HandleEngineStatusError(EEngineStatus::GAPI_NoPrimaryAdapterOutputModes, String::format("No valid output modes for output @index {} for adapter @index {} available.", outputIndex, adapterIndex));
              }
        ...
        { // Try to retrieve all adapters (graphics cards) and their output (monitors) and modes.
          ...
          while((res = pFactory->EnumAdapters(k, &pDxgiAdapterUnmanaged)) != DXGI_ERROR_NOT_FOUND) {
            ...
            // Get adapter information
            {
              ...
              if(FAILED(res)) {
                Log::Error(logTag(), String::format("Unable to fetch adapter description data for adapter {}", k));
              ...
              do {
                ...
                if(!FAILED(res)) {
                  ...
                  if(FAILED(res)) { // Did the data change again? This is too unstable... Abort gracefully.
                    Log::Error(logTag(), String::format("Failed to determine the output modes of 'device {}|adapter {}'", k, j));
            ...
            if(FAILED(res) && outAdapterOutputs.empty()) {
              if(outAdapters.size() < 1) {
                Log::Error(logTag(), String::format("Failed to get a valid output for adapter {}.", k));
