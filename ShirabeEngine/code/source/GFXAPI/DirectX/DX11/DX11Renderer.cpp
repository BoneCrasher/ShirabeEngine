	...
	namespace DX {
		namespace _11 {
			...
			EEngineStatus DX11Renderer::initialize(const ApplicationEnvironment &environment,
				...
				ResourceHandleList backBufferHandles;
				ResourceHandleList backBufferRTVHandles;
				...
				if (!CheckEngineError(status)) {
					...
					for (GAPIOutput output : primaryAdapter._outputs) {
						...
						for (GAPIOutputMode mode : output._outputModes) {
							if (mode._format != Format::RGBA_8_UNORM)
			...
			_createDeviceAndSwapChain:
				...
				dxRes = D3D11CreateDevice(NULL,                     // Adapter
				...
				swapChainDesc._name                     = "DefaultSwapChain";
				swapChainDesc._backBufferCount          = BACK_BUFFER_COUNT;
				swapChainDesc._texture._name            = "DefaultSwapChainTexture2D";
				swapChainDesc._texture._textureFormat   = outputMode._format;
				swapChainDesc._texture._dimensions[0]   = outputMode._size.x();
				swapChainDesc._texture._dimensions[1]   = outputMode._size.y();
				swapChainDesc._vsyncEnabled             = configuration._enableVSync;
				swapChainDesc._refreshRateNumerator     = outputMode._refreshRate.x();
				swapChainDesc._refreshRateDenominator   = outputMode._refreshRate.y();
				swapChainDesc._fullscreen               = !configuration._requestFullscreen;
				swapChainDesc._windowHandle             = reinterpret_cast<unsigned int>(static_cast<void *>(environment._primaryWindowHandle));
		
				status = resourceManager->createSwapChain(swapChainDesc, swapChainHandle, backBufferHandles);
				if (CheckEngineError(status)) {
					goto _return_failed;
				}
				
				// For each of the backbuffer textures, create an RTV.
				for (unsigned int k=0; k<BACK_BUFFER_COUNT; ++k) {
					status = resourceManager->createRenderTarget(backBufferHandles[k], backBufferRTVHandles[k]);
					if (CheckEngineError(status)) {
						// TODO: BAIL OUT
					}
				}
				...
				dsTexDesc     = {};
				dsTexDesc._dimensions             = { outputMode._size.x(), outputMode._size.y() };
				dsTexDesc._textureFormat          = Format::D24_UNORM_S8_UINT;
				dsTexDesc._mipMap._useMipMaps     = false;
				dsTexDesc._mipMap._mipLevels      = 1;
				dsTexDesc._array._isTextureArray  = false;
				dsTexDesc._array._textureCount    = 1;
				dsTexDesc._cpuGpuUsage            = ResourceUsage::CPU_None_GPU_ReadWrite;
				dsTexDesc._gpuBinding             = EToUnderlying(BufferBinding::ShaderOutput_DepthStencil);
				dsTexDesc._multisampling._count   = 1;
				dsTexDesc._multisampling._quality = 0;
